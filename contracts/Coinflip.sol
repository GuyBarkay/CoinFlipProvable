...
contract Coinflip is Ownable1, usingProvable {
  ...
  uint public ownerObligoToPlayers;
...
function flip () public  payable costs (0.05 ether )  {
...
/*function testRandom() public returns (bytes32){
 ...
 function getBalance() public view returns (uint){
  ...
  function getOwnerNetBalance() public view returns (uint){
...
function toTransferBet( ) public returns(uint) {
  ...
  function deposit() public onlyOwner payable  {
   acountBalance = address(this).balance-playerBalance[msg.sender];
